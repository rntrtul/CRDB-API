...
def makeCombatEncounter(camp_num):
  ...
  for row in reader:
    ...
    ce = CombatEncounter.objects.get_or_create(episode =ep, name=name, start=start, end=end,rounds=rounds,notes=note)
    if ce[1]:
  ...
  for encounter in ces:
    ...
    if order_sorted != {}:
      for counter, ord in enumerate(order_sorted.items()):
        rank = counter + 1
        init = InitiativeOrder.objects.get_or_create(encounter = encounter, character=ord[0], roll=ord[1], rank = rank)
        if init[1]:

...
from .serializers import (EpisodeSerializer, EpisodeDetailSerializer, ApperanceSerializer, AttendanceSerializer,
                          ApperanceTypeSerializer,
...
class DetailView(generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if context['object'].campaign.num == 1 and (
                context['object'].num == 31 or context['object'].num == 33 or context['object'].num == 35):
            context['yt_link'] = "https://youtu.be/" + yt_links[0].link_key + "?t="
            context['yt_link_p2'] = "https://youtu.be/" + yt_links[1].link_key + "?t="
            context['rolls'] = chain(context['object'].rolls.filter(notes__startswith='p1').order_by('timestamp'),
                                     context['object'].rolls.filter(notes__startswith='p2').order_by('timestamp'))
        else:
